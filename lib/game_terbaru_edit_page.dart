...
class GameTerbaruEditPage extends StatefulWidget {
  const GameTerbaruEditPage({Key? key}) : super(key: key);

  @override
  _GameTerbaruEditPageState createState() => _GameTerbaruEditPageState();
}

class _GameTerbaruEditPageState extends State<GameTerbaruEditPage> {
  ...
  void _simpanForm(context, id) async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await supabase.from('game_terbaru').update(data).match({'id': id});
  ...
  Widget build(BuildContext context) {
    final gb = ModalRoute.of(context)!.settings.arguments as Map;
    _kodegameController.text = gb['kode_game'];
    _namagameController.text = gb['nama_game'];
    _hargagameController.text = gb['harga_game'];
    _kategorigameController.text = gb['kategori_game'];
    _tanggalrilisController.text = gb['tanggal_rilis'];
    return Scaffold(
      appBar: AppBar(
        title: Text('Edit Game Terbaru'),
      ),
      body: Padding(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 10),
                ElevatedButton(
                  onPressed: () => _simpanForm(context, gb['id']),
                  child: Text('Simpan'))
              ],
            ),
          ),
