...
class GameTerlarisEditPage extends StatefulWidget {
  const GameTerlarisEditPage({Key? key}) : super(key: key);

  @override
  _GameTerlarisEditPageState createState() => _GameTerlarisEditPageState();
}

class _GameTerlarisEditPageState extends State<GameTerlarisEditPage> {
  ...
  void _simpanForm(context, id) async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await supabase.from('game_terlaris').update(data).match({'id': id});
  ...
  Widget build(BuildContext context) {
    final gl = ModalRoute.of(context)!.settings.arguments as Map;
    _kodegameController.text = gl['kode_game'];
    _namagameController.text = gl['nama_game'];
    _hargagameController.text = gl['harga_game'];
    _kategorigameController.text = gl['kategori_game'];
    _tanggalrilisController.text = gl['tanggal_rilis'];
    return Scaffold(
      appBar: AppBar(
        title: Text('Edit Game Terlaris'),
      ),
      body: Padding(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 10),
                ElevatedButton(
                  onPressed: () => _simpanForm(context, gl['id']),
                  child: Text('Simpan'))
              ],
            ),
          ),
