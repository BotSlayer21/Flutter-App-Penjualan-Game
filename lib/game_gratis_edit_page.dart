...
class GameGratisEditPage extends StatefulWidget {
  const GameGratisEditPage({Key? key}) : super(key: key);

  @override
  _GameGratisEditPageState createState() => _GameGratisEditPageState();
}

class _GameGratisEditPageState extends State<GameGratisEditPage> {
  ...
  void _simpanForm(context, id) async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await supabase.from('game_gratis').update(data).match({'id': id});
  ...
  Widget build(BuildContext context) {
    final gg = ModalRoute.of(context)!.settings.arguments as Map;
    _kodegameController.text = gg['kode_game'];
    _namagameController.text = gg['nama_game'];
    _hargagameController.text = gg['harga_game'];
    _kategorigameController.text = gg['kategori_game'];
    _tanggalrilisController.text = gg['tanggal_rilis'];
    return Scaffold(
        appBar: AppBar(
          title: Text('Edit Game Gratis'),
        ),
        body: Padding(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 10),
                ElevatedButton(
                  onPressed: () => _simpanForm(context, gg['id']),
                  child: Text('Simpan'))
              ],
            ),
          ),
